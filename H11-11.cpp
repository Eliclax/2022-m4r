...
long long int k = 11, n = 11;
// grid has coords (RIGHT, DOWN)
long long int grid[11][11], digitToAdd = 0, relIndex = 0, absIndex = 0, ii = 0, c = 0, jj = 0;
...
bool isDuped(long long int g[][11], long long int rows)
{
...
void find_row(long long int curRow)
{
    ...
    for (long long int i = 0; i < fact[n-1]; i++)
    {
        ...
        if (!isDuped(grid, curRow + 1))
        {
            ...
            if (curRow + 1 < 3) find_row(curRow + 1);
        }
    }
}
